...
export class RewardCalculator {
    ...
    async get_rewards(ssnaddr: string, delegator: string) {
        ...
        const last_reward_cycle_json = await this.contract.getSubState(KEY_LAST_REWARD_CYCLE);

        const reward_list = await this.get_reward_cycle_list(last_withdraw_cycle_map, last_reward_cycle_json, ssnaddr, delegator);
        const delegate_per_cycle = await this.combine_buff_direct(ssnaddr,delegator,reward_list);
        const need_list = await this.get_reward_need_cycle_list(last_withdraw_cycle_map, last_reward_cycle_json, ssnaddr, delegator);
